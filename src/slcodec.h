...
static SLSrr srr = {0};
static unsigned int stream_idx = 0;

void decode(SLSrr *srr)
{
    AVPacket packet;
    ...
    while (av_read_frame(ifmt_ctx, &packet) == 0) {
        if (packet.stream_index == stream_idx) {
            ...
            while ((ret = avcodec_receive_frame(stream_ctx->dec_ctx, stream_ctx->dec_frame)) == 0) {
                ...
                resampled_frame->ch_layout = stream_ctx->dec_frame->ch_layout;
                resampled_frame->format = AV_SAMPLE_FMT_FLT;
                
                swr_convert_frame(srr->srr_ctx, resampled_frame, stream_ctx->dec_frame);
...
static int open_input_file(const char *filename, SLSrr *srr)
{
    int ret;
    unsigned int i;
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (codec_ctx->codec_type == AVMEDIA_TYPE_VIDEO
            ...
            if (codec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
                codec_ctx->bit_rate = 192000; // 192 kbps
            }
